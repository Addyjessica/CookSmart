...
import {
  Box, Card, CardContent, CardCover, Stack,
} from '@mui/joy';
...
export default function MealModal({ open, setOpen, setData }) {
  ...
  const handleSave = () => {
    ...
    if (setData?.sections[0]?.components) {
      ...
      const ingredientPlurals = setData.sections[0]
        .components.map((component) => component.ingredient.display_plural);
      ...
      // Simulate loading state and redirect to the meal planner page
      setTimeout(() => {
        setLoad(false);
  ...
  return (
    ...
    >
      <Sheet
        variant="outlined"
        sx={{
          maxWidth: 800, borderRadius: 'md', p: 3, boxShadow: 'lg',
        }}
      >
          ...
          <Stack spacing={2}>
            {setData?.instructions.map((step) => (
              <Box
                key={step.id}
                sx={{
                  p: 2, border: '1px solid #ccc', borderRadius: 'md', backgroundColor: '#f9f9f9',
                }}
              >
